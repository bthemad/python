...
print(or_(user_table.c.username == 'edward', and_(user_table.c.fullname == 'ed', user_table.c.id > 5)))
...
insert_stmt = user_table.insert([
    {"username": "Dilbert", "fullname": "Dilbert Jones"}
])
conn.execute(insert_stmt)
conn.execute(select([user_table])).fetchall()

print(conn.execute(select([user_table]).where(
    or_(user_table.c.username == "wendy", user_table.c.username == "Dilbert"))).fetchall())
conn.execute(select([user_table])).fetchall()
...
#
update_stmt = user_table.update().values(fullname="Ed MaJones").where(user_table.c.username == "ed")
conn.execute(update_stmt)
conn.execute(select([user_table])).fetchall()

delete_stmt = user_table.delete().where(user_table.c.username == "Dilbert")
