...
from sqlalchemy import MetaData, Table, Column, String, Integer, select, update
...
ejoin = select([user_table.c.fullname, address_table.c.email_address]).select_from(user_table.join(address_table))
print ejoin

conn.execute(ejoin).fetchall()
...
#
### slide::
inner_select = select([address_table.c.email_address]).where(user_table.c.id == address_table.c.user_id)
conn.execute(user_table.select()).fetchall()
update_stmt = user_table.update().values(fullname=(user_table.c.fullname + inner_select)).where(user_table.c.username.in_(["jack", "wendy"]))
