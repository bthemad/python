...
network_table = Table('network', metadata,
                      ...
                      Column('crated_at', DateTime, nullable=False),
...
#
### slide::
network_reflected = Table('network', metadata2, autoload_with=engine)
print(network_table.c)

all_table_names = inspector.get_table_names()
lookup_column = "story_id"
found_table_names = []
for table_name in all_table_names:
    columns = inspector.get_columns(table_name)
    if any(column["name"] == u"story_id" for column in columns):
        print table_name