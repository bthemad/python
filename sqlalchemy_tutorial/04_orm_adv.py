...
session.query(Account.owner, Account.balance, Transaction.amount).join(Transaction).all()
subq = session.query(
                func.count(Address.id).label('count'),
                User.id.label('user_id')
                ).\
...
(session.query(Account.owner, Account.balance, func.sum(Transaction.amount).label('sum_total')).join(Transaction).group_by(Account.id).all())
### slide::