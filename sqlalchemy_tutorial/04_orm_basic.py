...
net1 = Network(name="net1")
session.add(net1)
session.add_all([Network(name="net2")])
session.query(Network).all()
session.commit()
...
query = session.query(User).order_by(User.fullname)
...
query = session.query(User).filter(or_(User.name== "mary", User.name== "ed"))
query.all()
